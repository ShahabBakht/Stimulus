...
function postPressDurationEffectStimulusFile()
...
distance2Screen = 50; % in cm
stimulusSize = 1; % in degree
stimulusContrasts = 0.92;%[0.028, 0.055, 0.11, 0.22, 0.46, 0.92];
postPressDuration = [.1,.2,.3,.4,.5];
numTrials = 20; % trials per condition
allConditions = repmat(postPressDuration,1,numTrials);
...
allCondTrialCount = zeros(length(postPressDuration),1);

for trcount = 1:(length(postPressDuration) * numTrials)
    thisPostPressDuration = allConditions(trialsOrder(trcount));
    [~,whichCond] = find(postPressDuration == thisPostPressDuration);
    allCondTrialCount(whichCond) = allCondTrialCount(whichCond) + 1;
    phase = 0;
    % Set Gabor parameters.
    propertiesMat = [phase, freq, sigma, stimulusContrasts, aspectRatio, 0, 0, 0];
    ...
    vbl = Screen('Flip',window);
    WaitSecs(thisPostPressDuration);%0.5
