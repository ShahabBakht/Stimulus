...
try
    ...
    numFrames       = 3;                   % how long to show the dots in seconds
    COHerences      = [0.15];%[0.7,0.8,0.9];       % test coherences %[0.1 0.3 0.5 .7 .9];
    view_dist_cm 	= 57;                  % distance to the screen in cm
    apDInDegree     = 6;                   % size of aperture in degrees
    speed           = 5;                   % dots speed in degree/sec
    dotSizeInDegree = .1;
    dotdensity      = 1;                   % dots / degree^2 in each frame
    numTrials       = 50;                  % number of trials per coherence level
    numGroupsOfDots = 1;
...
%     ndots = ceil(min(maxDotsPerFrame, ceil(16.7 * apD .* apD * 0.01 / monRefresh))/2);
    ...
    while continue_show
        ...
        if sum(~L_1) > 0  % if not 100% coherence
            ...
            this_s_1(~L_1,:) = this_s_1(~L_1,:) + dxdyRAND(~L_1,:); % displace along a random direction
        end
        if sum(~L_2) > 0  % if not 100% coherence
            % this_s_2(~L_2,:) = rand(sum(~L_2),2);	% get new random locations for the rest
            this_s_2(~L_2,:) = this_s_2(~L_2,:) + dxdyRAND(~L_2,:); % displace along a random direction
        end
    ...
    if ~gamepadUse
        ...
        while gamepadnotpressed
            ...
            if responseSate
                ...
                if buttonStateLeft
                    Response(trcount) = -1;
                    allResponse(whichCond,allCondTrialCount(whichCond)) = -1;
                else
                    Response(trcount) = 1;
                    allResponse(whichCond,allCondTrialCount(whichCond)) = 1;
                end
